...
router.post('/', authenticate, requirePermission('manage'), async (req, res) => {
  ...
  if (parent_id) {
    const [rows] = await pool.query<RowDataPacket[]>(
      'SELECT level FROM categories WHERE id = ?', [parent_id]
    );
    if (!rows.length) {
      res.status(400).json({ error: { code: 'NOT_FOUND', message: '父分类不存在' } });
      return;
    }
  ...
  try {
    await pool.query(
      ...
      [id, name, parent_id ?? null, level, sort_order]
    );
...
// PUT /api/categories/:id — 更新（manage 权限）
router.put('/:id', authenticate, requirePermission('manage'), async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      res.status(404).json({ error: { code: 'NOT_FOUND', message: '分类不存在' } });
      return;
    }
